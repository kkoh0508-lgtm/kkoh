...
function loginRequired(){
  cloudReady=false; main.inert=true;
...
async function api(path,method='GET',data){
  ...
  if(!response.ok){if(response.status===401&&path!=='login')loginRequired();throw Error(result.error||'서버 요청 실패');}
  return result;
}
...
async function cloudLoad(){
  ...
  try{
    ...
    cloudReady=true;main.inert=false;render();
...
$('loginForm').onsubmit=async e=>{
  e.preventDefault();const button=e.submitter;button.disabled=true;$('loginNotice').textContent='연결 중…';
  try{await api('login','POST',{password:$('password').value});if(await cloudLoad()){$('password').value='';$('loginDialog').close();loadHolidays();}else{$('loginNotice').textContent=$('cloudStatus').textContent;}}
  catch(error){$('loginNotice').textContent=error.message;}finally{button.disabled=false;}
};
$('reloadCloud').onclick=()=>cloudLoad();
$('logout').onclick=async()=>{
  if(cloudBusy)return;
  try{await api('logout','POST',{});events=[];holidayOverrides={};render();loginRequired();status('로그아웃되었습니다.');}catch(error){status(error.message);}
};
$('exportData').onclick=()=>{
...
setInterval(()=>{if(cloudReady&&!document.hidden&&!document.querySelector('dialog[open]'))cloudLoad();},30000);
window.addEventListener('online',()=>{if(cloudReady&&!document.querySelector('dialog[open]'))cloudLoad();});
window.addEventListener('beforeunload',e=>{if(cloudBusy){e.preventDefault();e.returnValue='';}});
(async()=>{if(await cloudLoad())loadHolidays();else loginRequired();})();
